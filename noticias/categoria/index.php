...
$result = mysqli_query($db_con, "SELECT id FROM noticias WHERE pagina LIKE '%2%' AND categoria = '$categoria' ORDER BY fechapub DESC");
...
$noticias = array();
$result = mysqli_query($db_con, "SELECT id, titulo, contenido, fechapub FROM noticias WHERE pagina LIKE '%2%' AND categoria = '$categoria' ORDER BY fechapub DESC LIMIT $query_sig, $limite");
while ($row = mysqli_fetch_array($result)) {
...
$pagina['titulo'] = 'Noticias por categoría <small>'.$categoria.'</small>';

// SEO
//$pagina['meta']['robots'] = 0;
//$pagina['meta']['canonical'] = 0;
$pagina['meta']['meta_title'] = $pagina['titulo'];
$pagina['meta']['meta_description'] = "Noticias y novedades de la categoria ".$categoria." en ".$config['centro_denominacion'];
