...
require("../plugins/pdf/mc_table.php");
...
class GranPDF extends PDF_MC_Table {
    ...
    function Header() {
        ...
        $this->Image( 'logo-fse.jpg',85,13,23,'','jpg');
        $this->SetY(15);
        $this->Cell(85);
  ...
  function Footer() {
        ...
        $this->SetTextColor(53, 110, 59);
        $this->Image( 'pie.jpg', 0, 250, 25, '', 'jpg' );
        $this->SetY(275);
        $this->SetFont('Noto Sans HK','',7);
        $this->Cell(75);
...
$result = mysqli_query($db_con, "SELECT `curso`, `unidad`, `apellidos`, `nombre`, `claveal`, `claveal1`, `numeroexpediente`, `combasi`, `primerapellidotutor`, `segundoapellidotutor`, `nombretutor`, `sexoprimertutor`, `domicilio`, `codpostal`, `localidad`, `provinciaresidencia`, `sexo`, `estadomatricula` FROM `alma` WHERE `claveal`='".$_SESSION['claveal']."' ORDER BY `curso` ASC, `unidad` ASC, `apellidos` ASC, `nombre` ASC");
...
foreach ($alumnos as $alumno) {
    ...
    $MiPDF->SetFont('Noto Sans HK', 'B', 8.5);
    $MiPDF->Cell(0, 5, 'Observaciones: '.$estadomatricula, 0, 1, 'L', 0);
    $MiPDF->SetFont('Noto Sans HK', '', 8.5);;
    $MiPDF->Ln(10);
    ...
    $MiPDF->Image('sello.jpg', 45, $MiPDF->GetY()-5, 35, '', 'jpg');
