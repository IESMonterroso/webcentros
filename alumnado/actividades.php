...
<div class="table-responsive">
	<table class="table table-bordered table-striped">
		...
		<tbody>
			...
			<tr>
				<td>
					<h4><span class="text-warning"><?php echo stripslashes($row_actividad['nombre']); ?></span><br><?php if(($row_actividad['fechaini'] == $row_actividad['fechafin']) && ($row_actividad['horaini'] == $row_actividad['horafin'])): ?><small class="text-muted">Todo el día</small><?php else: ?><small class="text-muted">Salida: <?php echo strftime('%e de %B',strtotime($row_actividad['fechaini'])); ?> a las <?php echo strftime('%H:%M',strtotime($row_actividad['horaini'])); ?> | Regreso: <?php echo strftime('%A, %e de %B',strtotime($row_actividad['fechafin'])); ?> a las <?php echo strftime('%H:%M',strtotime($row_actividad['horafin'])); ?></small><?php endif; ?></h4>
